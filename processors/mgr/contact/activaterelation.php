...
if ($modx->lexicon) {
   ...
   switch ($scriptProperties['task']) {
        case 'activate':
            if ($joinobject = $modx->getObject($joinclass, array('form' => $scriptProperties['co_id'], 'contact' => $scriptProperties['object_id']))) {
                break;
            } else {
                $joinobject = $modx->newObject($joinclass);
                $joinobject->set('form', $scriptProperties['co_id']);
                $joinobject->set('contact', $scriptProperties['object_id']);
            }
            $joinobject->save();
            break;
        case 'deactivate':
